...
class EntryWindow(Gtk.Window):
    def __init__(self):
        ...
        headerbar.set_title("Transub")
        headerbar.set_subtitle("It's in beta phase, so hehh")
    ...
    ##The credit text shown at startup
        ...
        textbuffer.insert(textbuffer.get_start_iter(), "=============================================\n=== Project made by SuperYang and forked by EnderDatsIt ===\n=============================================\n")
...
##The action that the start button does
    ...
    ##The part that gets the select language to translate in
        ...
        ##The part that translates
            ...
            translation_thread = Thread(target=translate_subtitles, args=(self, self.file, target_language, progress_queue))
...
def on_settings_button_clicked(button):
    ...
    pixbuf = GdkPixbuf.Pixbuf.new_from_file_at_scale(
                filename="./resources/icons/about_icon.png", 
                width=64, 
                height=64, 
    ...
    label = Gtk.Label(label="This project is a subtitle translation tool developed by SuperYang\nand forked by EnderDatsIt. It uses Google Translate and DeepL API to translate subtitles\nand pysubs2 library for parsing subtitle files.\n\nVersion: " + version)
...
def translate_subtitles(self, input_file, target_language, queue):

##Checks for file format
    if not self.file:
        return

    if self.file.lower().endswith('.srt'):
        subs_format = 'srt'
    elif self.file.lower().endswith('.ass'):
        subs_format = 'ass'
    else:
